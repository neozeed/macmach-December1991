...

#ifdef CMU
/* When CMU is defined, additional code is compiled to implement the
 * CMU CS Departement version of CPATH/MAKEPSD/MAKECPP.  The CMU search
 * path for include files looks like:
 * {./}{MAKEPSD or MAKECPP} *** {-I} {CPATH or defaults}
 * Where ./ is the source file's directory, MAKEPSD and MAKECPP are
 * environment variables (MAKECPP overrides MAKEPSD), *** is the beginning
 * of the search for <> include files and CPATH is an environment variable
 * which will override the compiled-in defaults.  See gcc.c for the CMU
 * implementation of LPATH.
 * Additionally, the CMU -MD switch is implemented which creates *.d files
 * with the dependency information while compiling as usual.
 */
#endif /* CMU */
...
#ifdef CMU
/* 3 means create "*.d" file and output as normal */
#endif /* CMU */

int print_deps = 0;

#ifdef CMU
/* Nonzero means display include file debugging information */
...
#ifdef CMU
  /* if non-zero, dep output file was specified on command line */
  md_fname = 0;

  /* set include debug flag if environment variable exists */
  cpp_incl_debug = (getenv("CPP_INCL_DEBUG") != NULL);
#endif /* CMU */
...
#ifdef CMU
  /* Add directories from MAKEPSD or MAKECPP environment variables. */
  if ((makecpp = getenv("MAKECPP")) || (makecpp = getenv("MAKEPSD"))) {
    struct file_name_list *dirtmp;
    char *n;
    int nn;

    while (*makecpp && *makecpp == ':') makecpp++;
    while (*makecpp) {
      n = makecpp;
      makecpp = (char *)index(makecpp, ':');
      if (!makecpp) {
      	nn = strlen(n);
      	makecpp = &n[nn];
      }
      ...
      if (include == 0) include = dirtmp;
      else last_include->next = dirtmp;
      last_include = dirtmp;	/* Tail follows the last one */
      dirtmp->fname = xmalloc(nn + 1);
      strncpy(dirtmp->fname, n, nn);
      dirtmp->fname[nn] = '\000';
...
#ifdef CMU
	{
	  if ((print_deps == 3) && !md_fname) md_fname = argv[i];
	  else fatal("Usage: %s [switches] input output [MD output]", argv[0]);
	}
#else /* CMU */
...
#ifdef CMU
  /* display include list if debugging */
  if (cpp_incl_debug) {
    struct file_name_list *dirtmp = include;
    int i;
    for (i = 0; dirtmp; i++, dirtmp = dirtmp->next) {
      fprintf(stderr, "include #%02d = \"%s\"", i, dirtmp->fname);
      if (dirtmp == first_bracket_include) fprintf(stderr, " <***>");
      fprintf(stderr, "\n");
    }
  }
#endif /* CMU */
...
#ifdef CMU
  /* If the -MD option specified, output the deps to md_fname */
  else if (print_deps == 3) {
    if (!md_fname) {
      extern char *rindex ();
      char *n, *p;

      /* if md_fname not specified, use "<input file>.d". */
      if (!(n = (char *)xmalloc(strlen(in_fname) + 2)))
        pfatal_with_name(in_fname);
      strcpy(n, in_fname);
      /* The usual case is to turn "foo.c" into "foo.d". */
      p = rindex(n, '.');
      if (!p || !p[1] || (p[1] == 'd' && !p[2]))
        fatal("Bad file name for -MD");
      p[1] = 'd';
      p[2] = '\000';
      /* Remove any directory preface from name -- write local file. */
      if (p = rindex(n, '/')) strcpy(n, p + 1);
      md_fname = n;
    }
    if (!(deps_stream = fopen(md_fname, "w")))
      pfatal_with_name(md_fname);
  }
#endif /* CMU */
...
get_filename:
  ...
  if (cpp_incl_debug) {
    ...
    fname[flen] = '\000';
    fprintf(stderr, "include %c%s%c\n",
      system_header_p ? '<' : '"', fname, system_header_p ? '>' : '"'
    );
  }
#endif /* CMU */
...
#ifdef CMU
      if (cpp_incl_debug) fprintf(stderr, "  trying \"%s\"\n", fname);
#endif /* CMU */
