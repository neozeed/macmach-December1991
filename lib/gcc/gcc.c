...
#ifdef CMU
/* handle -MD switch and -E with a .s file */
{
  {".c",
  ...
  {".s",
   "%{!E:%{!S:as %{R} %{j} %{J} %{h} %{d2} %a \
    ...
    %{E:cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{trigraphs} \
        -undef -D__GNUC__ -$ %p %P\
        %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\
	%{Wcomment*} %{Wtrigraphs} %{Wall} %C %i\n }"},
  {".S",
...
#ifdef DEFAULT_PIPE
  /* yes this is a hack, but it should do the right thing without breaking */
  argv[argc++] = "-pipe";
#endif
  process_command (argc, argv);
