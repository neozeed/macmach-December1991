 ...
 * HISTORY
 * $Log:	rmt_dev.c,v $
 * Revision 2.2  90/06/02  15:29:18  rpd
 * 	Fixed buggy rmtwrite/rmt_send interaction that led to
 * 	a "RT_WBUF attached" panic.
 * 	[90/04/15            rpd]
 * 
 * Revision 2.1  89/08/04  15:11:03  rwd
 * Created.
 * 
...
#if	MACH_NO_KERNEL
   selwakeup((caddr_t)&rmt.rt_selproc);
#else	MACH_NO_KERNEL
#if	MACH
   if (rmt.rt_selproc)
#else	MACH
...
#if	MACH_NO_KERNEL
       sleep(&rmt.rt_reply, sendSignal ? RMT_NOINT_PRI : RMT_INTOK_PRI);
#else	MACH_NO_KERNEL
    ...
    int flag;
{
	...
	case FREAD:
		if (rmt.rt_flags&RT_RBUF)
			return(1);
#if	MACH_NO_KERNEL
		selenter((caddr_t)&rmt.rt_selproc);
#else	MACH_NO_KERNEL
#if	MACH
    ...
    struct uio *uio;	/* standard unix i/o structure for a device driver */
{
    ...
    /*
     *	Problem: the uiomove can be interrupted.
     *	A process coming into rmt_send can see RT_WBUF on,
     *	with its own pid left in rmt.rt_imw.im_client, and panic
     *	because rmt_wbuf_attached() is true.
     *	Hack solution: set im_client to something invalid.
     */
    rmt.rt_flags |= RT_WBUF;
    rmt.rt_imw.im_client = 0;
