 ...
 * Revision 2.2  90/06/19  23:09:59  rpd
 * 	Changed sowakeup to use psignal3/gsignal3.
 * 	[90/06/09            rpd]
 * 
 * 	Out-of-kernel version.  Use new select mechanism.
 * 	[89/01/05            dbg]
 * 
 * Revision 2.1  89/08/04  14:16:02  rwd
 * Created.
 * 
 * Revision 2.4  88/08/24  01:38:50  mwyoung
 * 	Corrected include file references.
 * 	[88/08/22  22:06:09  mwyoung]
 * 
 *
 * 29-Mar-88  Michael Young (mwyoung) at Carnegie-Mellon University
 *	CS_GENERIC: Don't compile dead code.
 *
 * 26-Oct-87  David Golub (dbg) at Carnegie-Mellon University
 *	Oops... select routines really do have to check whether
 *	the thread is waiting.  sb_sel could be non-zero from
 *	the last time...
 *
...
{
#if	MACH_NO_KERNEL
	selenter((caddr_t)&sb->sb_sel);
#else	MACH_NO_KERNEL
#if	MACH
	if (sb->sb_sel &&
			((thread_t)(sb->sb_sel))->wait_event == (int)&selwait)
#else	MACH
	...
	struct sockbuf *sb;
{
	...
	if (so->so_state & SS_ASYNC) {
		if (so->so_pgrp < 0)
			gsignal3(-so->so_pgrp, SIGIO, FALSE);
		else if (so->so_pgrp > 0 && (p = pfind(so->so_pgrp)) != 0)
			psignal3(p, SIGIO, FALSE);
	}
}
