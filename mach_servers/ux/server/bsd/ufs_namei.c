 ...
 * HISTORY
 * $Log:	ufs_namei.c,v $
 * Revision 2.2  90/01/23  00:04:08  af
 * 	Added namei macros [from original ITC idea].
 * 	"@sys" expands to the machine's system name (e.g."pmax_mach")
 * 	"@cpu" expands to the machine's cpu name (e.g. "mips").
 * 	Actual definitions are in a separate include file.
 * 	[89/12/15            af]
 * 
 * Revision 2.1  89/08/04  14:15:10  rwd
 * Created.
 * 
...
startdir:
	 ...
	 *  directory).
	 */
	if ((u.u_rfs&i) && okremote && u.u_rfscode != 0)
		goto rfsdir;
#endif	CMUCS_RFS
#if	CMUCS
...
#if	NAMEI_MACROS
	if (ndp->ni_dent.d_name[0] == '@') {
		if (i == (sizeof ATSYS - 1) &&
		    strcmp(ndp->ni_dent.d_name,ATSYS) == 0) {
			copystr(SYSNAME,ndp->ni_dent.d_name,SYSNAMELEN+1, 0);
			ndp->ni_dent.d_namlen = i = SYSNAMELEN;
		} else if (i == (sizeof ATCPU - 1) &&
		    strcmp(ndp->ni_dent.d_name,ATCPU) == 0) {
			copystr(CPUNAME,ndp->ni_dent.d_name,CPUNAMELEN+1, 0);
			ndp->ni_dent.d_namlen = i = CPUNAMELEN;
		}
	}
#endif	NAMEI_MACROS
	isdotdot = (i == 2 &&
...
found:
	...
	if (!(okremote &&
		...
		quoted < QUOTED &&
#endif	CMUCS
		u.u_rfscode != 0 && isrfslnk(dp)) && makeentry) {
#else	CMUCS_RFS
	if (makeentry) {
#endif	CMUCS_RFS
		if (ncp != NULL)
