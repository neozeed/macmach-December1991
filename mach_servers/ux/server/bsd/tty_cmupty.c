 ...
 * HISTORY
 * $Log:	tty_cmupty.c,v $
 * Revision 2.2  90/06/19  23:09:10  rpd
 * 	Purged MACH, MACH_NO_KERNEL conditionals.
 * 	[90/06/08            rpd]
 * 
 ...
 * 	[89/01/05            dbg]
 * 
 * Revision 2.1  89/08/04  14:13:37  rwd
 * Created.
 * 
...
register struct tty *tp;
{
	...
	else if (ptywread(ccp, tp))
	{
		...
		selwakeup((caddr_t)&ccp->pt_rselp);
	}
...
dev_t dev;
{
	...
	case PIOCENBS:
	{
	    ...
	    if (*((u_int *)data) <= NSIG)
	    {
		ipl = spltty();		/* This probably isn't needed */
