 ...
 * HISTORY
 * $Log:	tty.c,v $
 * Revision 2.5  91/03/20  15:01:58  dbg
 * 	Fixed IOCTLs for ANSI C.
 * 	[91/02/21            dbg]
 * 
 * Revision 2.4  90/12/05  18:47:23  af
 * 	Modified the dropping of input at TTYHOG into TTHIWAT,
 * 	which depends on speed.  This ameliorates the situation
 * 	a lot at e.g. 9600 baud, but we still drop characters.
 * 	The problem appears to be pushing them chars along to
 * 	the user not fast enough.
 * 
 * 
 * Revision 2.3  90/06/19  23:08:54  rpd
 * 	Changed gsignal in ttyinput and ttwakeup to gsignal3.
 * 	Purged CMUCS, MACH, MACH_NO_KERNEL conditionals.
 * 	[90/06/10            rpd]
 * 
 * Revision 2.2  89/11/29  15:27:53  af
 * 	Out-of-kernel version.
 * 	Use new select mechanism.
 * 	Use tty hash table to find tp from dev number.
 * 	[89/01/05            dbg]
 * 
 * Revision 2.1  89/08/04  14:07:24  rwd
 * Created.
 * 
 ...
 *	Also, merged VICE & romp versions of tty console switching code.
 ...
 *	romp: added changes from ACIS for RT cons_tp variable support.
 *
...
/*
 *  Console terminal ouptut redirect hack
 */
	...
	int rw;
{
	...
	case FREAD:
		...
		if ((nread > 0) || ((tp->t_state & TS_CARR_ON) == 0))
			goto win;
		selenter((caddr_t)&tp->t_rsel);
		break;

	case FWRITE:
		if (tp->t_outq.c_cc <= TTLOWAT(tp))
			goto win;
		selenter((caddr_t)&tp->t_wsel);
		break;
	}
	splx(s);
	return (0);
	...
	register struct tty *tp;
{
	...
	if (tp->t_line == NTTYDISC) {
		...
		if (c == tp->t_suspc) {
			if ((t_flags&NOFLSH) == 0)
				ttyflush(tp, FREAD);
			ttyecho(c, tp);
			gsignal3(tp->t_pgrp, SIGTSTP, FALSE);
			goto endcase;
		}
	}
	...
	if (c == tp->t_intrc || c == tp->t_quitc) {
		if ((t_flags&NOFLSH) == 0)
			ttyflush(tp, FREAD|FWRITE);
		ttyecho(c, tp);
		gsignal3(tp->t_pgrp, c == tp->t_intrc ? SIGINT : SIGQUIT, FALSE);
		goto endcase;
	}
	 ...
	 */
	if (tp->t_rawq.c_cc+tp->t_canq.c_cc >= TTHIWAT(tp)) {
		if (tp->t_line == NTTYDISC)
#ifdef mac2 /* tahoe compatability */
			(void) ttyoutput(CTRL('g'), tp);
#else
...
ttwakeup(tp)
	struct tty *tp;
{
	selwakeup((caddr_t)&tp->t_rsel);
	if (tp->t_state & TS_ASYNC)
		gsignal3(tp->t_pgrp, SIGIO, FALSE);
	wakeup((caddr_t)&tp->t_rawq);
}
