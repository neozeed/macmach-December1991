 ...
 * HISTORY
 * $Log:	uipc_socket.c,v $
 * Revision 2.2  90/08/06  15:32:37  rwd
 * 	sohasoutofband must call {pg}ignal3 with block = FALSE since it
 * 	can happen at interrupt.
 * 	[90/07/24            rwd]
 * 
 * 	Out-of-kernel version.
 * 	[89/01/06            dbg]
 * 
 * Revision 2.1  89/08/04  14:15:54  rwd
 * Created.
 * 
	...
	int proto;
{
	...
	so->so_type = type;
#if	CMUCS
	if (u.u_uid == 0 || groupmember(38))	/* XXX */
#else	CMUCS
	if (u.u_uid == 0)
#endif	CMUCS
		so->so_state = SS_PRIV;
...
#else	LESS STUPID
			if (uio->uio_resid >= 2 * MLEN) {
				MCLGET(m);
				if (m->m_len != CLBYTES)
					goto nopages;
				len = MIN(MIN(CLBYTES, uio->uio_resid), space);
			} else {
nopages:
				len = MIN(MIN(MLEN, uio->uio_resid), space);
			}
	...
	register struct socket *so;
{
	...
	if (so->so_pgrp < 0)
		gsignal3(-so->so_pgrp, SIGURG, FALSE);
	else if (so->so_pgrp > 0 && (p = pfind(so->so_pgrp)) != 0)
		psignal3(p, SIGURG, FALSE);
#if	MACH_NO_KERNEL
	selwakeup((caddr_t)&so->so_rcv.sb_sel);
#else	MACH_NO_KERNEL
	if (so->so_rcv.sb_sel) {
