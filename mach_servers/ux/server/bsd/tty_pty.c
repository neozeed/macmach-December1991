 ...
 *
 * $Log:	tty_pty.c,v $
 * Revision 2.1  89/08/04  14:13:54  rwd
 * Created.
 * 
 ...
 */
	...
	case TIOCSSIZE:	/* This isn't very pretty, but what driver is? */
		 ...
		 * by exmining the "ts_lines" field in the ttysize structure.
	...
	case _N_TIOCSSIZE:
		tp->t_winsize.ws_row = ((struct ttysize *)data)->ts_lines;
		tp->t_winsize.ws_col = ((struct ttysize *)data)->ts_cols;
		return (0);

	case TIOCGSIZE:
	case _N_TIOCGSIZE:
		((struct ttysize *)data)->ts_lines = tp->t_winsize.ws_row;
		((struct ttysize *)data)->ts_cols = tp->t_winsize.ws_col;
		return (0);
        }
