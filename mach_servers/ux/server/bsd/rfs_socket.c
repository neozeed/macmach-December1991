 ...
 * HISTORY
 * $Log:	rfs_socket.c,v $
 * Revision 2.3  90/08/06  15:32:09  rwd
 * 	Fix references to share_lock.
 * 	[90/06/08            rwd]
 * 
 * Revision 2.2  90/03/14  21:27:01  rwd
 * 	Change shared locks to use share_lock macros.
 * 	[90/02/16            rwd]
 * 	Added MAP_UAREA code
 * 	[90/01/31            rwd]
 * 
 * Revision 2.1  89/08/04  14:09:54  rwd
 * Created.
 * 
...
#if	MAP_UAREA
    share_lock(&p->p_siglock, p);
#else	MAP_UAREA
    mutex_lock(&p->p_siglock);
#endif	MAP_UAREA
    mask = p->p_sigmask;
    p->p_sigmask = -1;
#if	MAP_UAREA
    share_unlock(&p->p_siglock, p);
#else	MAP_UAREA
    mutex_unlock(&p->p_siglock);
#endif	MAP_UAREA
