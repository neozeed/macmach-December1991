...
#ifdef MACH
  /*
   * MACH 4.3BSD systems use the TABLE(2) system call to get load average
   */
  {
    int i;
    struct {
    ...
    if (table(TBL_LOADAVG, 0, &tbl_loadavg, 1, sizeof(tbl_loadavg)) == 1)
      for (i = 0; i < 3; i++) {
        if (!tbl_loadavg.tl_lscale)
          load_ave[i] = tbl_loadavg.tl_un.tl_davenrun[i];
        else
          load_ave[i] = ((double)(tbl_loadavg.tl_un.tl_lavenrun[i])) /
