...
extern int hcflg, binary, fncnv, local, server, cxseen, czseen;
...
input() {
    ...
    debug(F101,"input sstate","",(int) sstate);
...
/* No need to set ACK'd bit, because we're gonna free the buffer now */
    ...
    spack('Y',n,strlen((char *)s),s);		/* Now send it. */
...
/* New packet, not seen before, enter it into the "database". */
    ...
    screen(SCR_PT,type,(long)rsn,(char *)sohp);	/* Update screen */
