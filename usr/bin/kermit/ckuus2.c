...
extern char line[], pktfil[], sesfil[], ttname[];
extern char *xarg0;
extern int nrmt, nprm, dfloc, seslog, local, parity, duplex;
...
static char *hmxxlg[] = { "Syntax: LOG name [ { NEW, APPEND } ]",
...
static char *hmhskey[] = { "Syntax: SET KEY n m\n",
"Specify that the key whose code is n (0 - 255) should send the code m",
"(0 - 255) when pressed during CONNECT mode.  n and m can be expressed as",
"decimal numbers or backslash codes.  SET KEY mappings take place before",
"terminal character-set translation.",
""};
...
case XXHLP:
    ...
    printf("%s, Copyright (C) 1985, 1991,\n",versio);
    return(hmsga(tophlp));
...
case XXHLP:
    printf("%s, Copyright (C) 1985, 1991,",versio);
    return(hmsga(tophlp));
...
#ifdef OS2
"SET TERMINAL VT100 or TEK4014, to set terminal emulation mode (OS/2 only).",
#endif /* OS2 */
