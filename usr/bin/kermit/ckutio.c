char *ckxv = "UNIX tty I/O, 5A(082), 4 Nov 91";
...
#ifdef POSIX				/* NOTE: should move this */
#ifndef SVR4ORPOSIX			/* to ckcdeb.h... */
#define SVR4ORPOSIX
#endif /* SVR4ORPOSIX */
#endif /* POSIX */
#ifdef SVR4
...
#ifdef ACUCNTRL
_PROTOTYP( int acucntrl, (char *, char *) );
#endif /* ACUCNTRL */
...
/*
  ...
  This was tested with TCP/IP connections and did not work.  Furthermore,
  the use of a numeric "device name" conflicts with X.25 SET HOST (-j)
  commands (which use numeric addresses).  Hence the limitation to serial
  connections only.
*/
    if (modem > -1) {			/* Only for serial connections */
...
/* This is definitely dangerous.  Why was it here? */
    ...
    return(z < 0 ? -1 : 1);
#else
    return(x);				/* Just return last ioctl status */
#endif /* COMMENT */
#endif /* ANYBSD */
...
#ifdef RTSXOFF
	/* This is the preferred way, according to SVID R3 */
...
/* Prototype obtained from system header files for POSIX compilations. */
...
#ifdef TCXONC				/* Maybe we're xoff'd */
	    debug(F100,"ttoc TCXONC","",0); 
	    ioctl(ttyfd, TCXONC, 1);	/* Try to clear it up the new way. */
#else
#ifdef BSD4				/* Or else try old Berkeley way. */
#ifdef TIOCSTART
