char *fnsv = "C-Kermit functions, 5A(057) 4 Nov 91";
...
extern int pktnum, sndtyp, bctr, bctu, fmask, clfils, sbufnum,
 size, osize, spktl, nfils, stdouf, warn, timef, spsizf;
extern int parity, turn, turnch, network, what,
...
CHAR ident();				/* Identity translation function */
#endif /* NOCSETS */
...
#endif /* KANJI */
	      if (xlate && rx) a = (*rx)(a); /* Translate if requested */
#endif /* NOCSETS */
    	}
...
getpkt(bufmax,xlate) int bufmax, xlate; { /* Fill one packet buffer */
	...
	ffc = 0;		/* Reset file character counter */
...
/* sipkt */ {
    ...
    return(spack(c,pktnum,strlen((char *)rp),rp)); /* Send them. */
}
...
sfile(x) int x; {
    ...
    spack(x ? 'X' : 'F', pktnum, size, encbuf+7); /* Send the F or X packet */
...
#ifdef COMMENT
	    /* Why???  It's per packet, not cumulative. */
	    if (wslotn >= maxtry) maxtry = wslotn + 1;
#endif
	} else {
