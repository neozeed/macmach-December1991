...
extern int size, rpsiz, urpsiz, local, backgrd, bgset, tvtflg, sosi, suspend,
  server, displa, binary, parity, deblog, escape, xargc, xargs, flow, cmdmsk,
  turn, duplex, nfils, ckxech, pktlog, seslog, tralog, stdouf, stdinf, what,
...
extern CHAR sstate, keymap[];
extern int rmailf, rprintf, cmflgs, techo, terror, repars, ncmd;
extern char savbuf[];
...
#ifdef COMMENT
/* No more.  Now this is done in sysinit(). */
    if (congm() < 0)
      printf("ERROR GETTING CONSOLE MODES\n");
#endif /* COMMENT */
...
addmmac(nam,s) char *nam, *s[]; {	/* Add a multiline macro definition */
    int i, x, y; char *p;
    x = 0;				/* Length counter */
    for (i = 0; (y = strlen(s[i])) > 0; i++) { /* Add up total length */
...
addmac(nam,def) char *nam, *def; {	/* Add a macro to the macro table */
    ...
    if (!nam) return(-1);
    namlen = strlen(nam);		/* Get argument lengths */
...
popclvl() {				/* Pop command level, return cmdlvl */
	...
	bgchk();			/* Check background status */
	concb(escape);			/* Go into cbreak mode */
    }
...
int popclvl() {				/* Just close current take file. */
	...
	conint(trap,stptrap);		/* check and set interrupts */
        ...
        concb(escape);			/* and go back into cbreak mode. */
    }
    return(tlevel + 1);
}
#endif /* NOSPL */
...
prtopt(s) char *s; {			/* Print an option */
    ...
    y = strlen(s);			/* reset horizontal position. */
    x += y;
    if (x > 79) {
	printf("\n%s",s);
	x = y;
