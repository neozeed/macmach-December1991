...
/* See copyright notice at top of ckcfns.c. */
...
extern int binary, rprintf, rmailf, spsiz;
...
sattr(xp) int xp; {			/* Send Attributes */
	...
	if (atleno) {
	    ...
	    aln = strlen(&data[i+2]);
#else
	    aln = strlen((char *)(data+i+2));
#endif
	    data[i+1] = tochar(aln);
	    i += aln + 2;

	    data[i] = '1';			/* File length in bytes */
	    sprintf((char *) &data[i+2],"%ld",fsize);
#ifdef COMMENT
	    aln = strlen(&data[i+2]);
#else
	    aln = strlen((char *)(data+i+2));
    ...
    aln = strlen((char *)data);		/* Get overall length of attributes */
