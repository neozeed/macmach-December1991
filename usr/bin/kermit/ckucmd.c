...
char *cmdv = "Unix cmd package 5A(046), 2 Nov 91";
...
_PROTOTYP( VOID clrhlp, () );
_PROTOTYP( VOID dmphlp, () );
_PROTOTYP( int gtword, () );
_PROTOTYP( int addbuf, (char *) );
_PROTOTYP( int setatm, (char *) );
_PROTOTYP( int cmdgetc, () );
_PROTOTYP( VOID cmdnewl, (char) );
_PROTOTYP( VOID cmdchardel, () );
_PROTOTYP( VOID cmdecho, () );
...
cmpush() {				/* Save the command environment */
	...
	fatal("cmpush: no memory for cmp", 0);
...
#ifdef DCMDBUF
    cp = malloc(strlen(cmdbuf)+1);	/* 0: Command buffer */
    if (cp) strcpy(cp,cmdbuf);
    cmp[cmddep].b[0] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(savbuf)+1);	/* 1: Save buffer */
    if (cp) strcpy(cp,savbuf);
    cmp[cmddep].b[1] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(hlpbuf)+1);	/* 2: Help string buffer */
    if (cp) strcpy(cp,hlpbuf);
    cmp[cmddep].b[2] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(atmbuf)+1);	/* 3: Atom buffer */
    if (cp) strcpy(cp,atmbuf);
    cmp[cmddep].b[3] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(atxbuf)+1);	/* 4: Expansion buffer */
    if (cp) strcpy(cp,atxbuf);
    cmp[cmddep].b[4] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(atybuf)+1);	/* 5: Atom buffer copy */
    if (cp) strcpy(cp,atybuf);
    cmp[cmddep].b[5] = cp;
    if (cp == NULL) return(-1);

    cp = malloc(strlen(filbuf)+1);	/* 6: File name buffer */
...
/* don't need this stuff, zxpand does it now. */
		...
		*xp = atmbuf;
                ...
                *wild = iswild(*xp);
                ...
                if (*wild != 0) {
#endif /* COMMENT */
		    sv = malloc(strlen(*xp)+1);	/* Make a safe copy */
		    if (!sv) {
