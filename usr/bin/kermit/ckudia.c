#ifndef NODIAL
char *dialv = "Dial Command, 5A(025) 2 Nov 91";
...
MDMINF HST =		/* information for Courier HST modem */
    ...
    ",",		/* pause_chars */
    2,			/* pause_time */
    "AAAAAAAAAATE1Q0TS2=43X7&B1&M4\015", /* wake_str */
    0,		        /* wake_rate */
    "OK\015",		/* wake_prompt */
    "",		        /* dmode_str */
    "",		        /* dmode_prompt */
...
#ifdef OS2				/* arg req'd for ANSI C signal() */
    alarmack();				/* prototype... */
#endif /* OS2 */
...
static VOID
dialoc(c) char c; {			/* Dial Output Character */
    if (dialdpy) {
...
*/
	...
	longjmp( sjbuf, status == 0 ? F_minit : F_modem);
...
*/
	...
	if (status < 1) longjmp( sjbuf, F_minit ); /* Failed. */
	...
	if (!dialini) {			/* If using built-in init strings */
		...
		if (status < 1) longjmp(sjbuf, F_minit);
	    }
	}
...
*/
		...
		*/
		...
		if (strlen(nbuf) > 3)	/* Probably phone number echoing... */
		  continue;
		/*
