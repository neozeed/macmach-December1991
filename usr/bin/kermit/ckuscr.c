#ifndef NOICP
#ifndef NOSCRIPT
char *loginv = "Script Command, 5A(011) 1 Oct 91";
...
extern int local, flow, seslog, mdmtyp, quiet, duplex, network, backgrd;
extern long speed;
extern char ttname[];
...
#ifdef OS2				/* arg req'd for signal() prototype */
...
dologin(cmdstr) char *cmdstr; {
	...
	scr_echo = (!quiet && !backgrd); /* Whether to echo script cmds */
#ifndef NOSPL
...
#endif /* NOSPL */
    	if (scr_echo)
	  printf("Executing script thru %s, speed %ld.\n",ttname,speed);
	*seq_buf=0;
...
/* start expect - send sequence */
    ...
    if (!quiet && !backgrd) printf("Script successful.\n");
    tlog(F100,"Script successful.","",0L);
    return(1);

failret:
    signal(SIGALRM,savealm);
    if (!quiet && !backgrd) printf("Sorry, script failed\n");
