char *cknetv = "Network support, 5A(003) 4 Nov 91";
...
/*
 ...
 Copyright (C) 1991, Trustees of Columbia University in the City of New 
...
extern int debses, seslog, ttyfd;	/* External variables */
...
#define X29PID 1			/* X.29 Protocol ID */
	...
	/* Set call user data if specified */
        if (cudata) {
            strncpy(x25host.data+X29PIDLEN,udata,strlen(udata));
            x25host.datalen += strlen(udata);
        }
...
*/
	...
	*p++ = '\0';			/* Get service name or number */
    ...
    debug(F101,"netopen service","",(int) service);
    if (!service) {
...
#ifdef OXOS
/* In Olivetti X/OS, inet_addr()'s type is "struct inaddr" */
      if ((saddr.sin_addr.s_addr = *(unsigned long *)&inet_addr(namecopy))
	  != ((unsigned long)-1))
#else
#ifdef DU2
/* Ditto for Ultrix 2.0 */
      if ((saddr.sin_addr.s_addr = *(unsigned long *)&inet_addr(namecopy))
	  != ((unsigned long)-1))
#else
/* Elsewhere (?) it's unsigned long */
      if ((saddr.sin_addr.s_addr = inet_addr(namecopy)) != ((unsigned long)-1))
#endif /* DU2 */
#endif /* OXOS */
...
#else
	debug(F101,"netopen connect errno","",errno);
	if (errno) perror("netopen connect");
#endif /* MULTINET */
#endif /* EXCELAN */
	return(-1);
    }
...
static int sgaflg;			/* telnet SGA flag */
...
tn_ini() {
    ...
    return(0);
#else
    debug(F101,"tn_ini ttnet","",ttnet);
    debug(F101,"tn_ini NET_TCPB","",NET_TCPB);
    debug(F101,"tn_ini ttnproto","",ttnproto);
    debug(F101,"tn_ini NP_TELNET","",NP_TELNET);
    ...
    if (ttnproto != NP_TELNET)		/* Be sure we're talking telnet. */
      return(0);
    if (tn_init != 0)			/* Have we done this already? */
      return(0);
...
*/
#ifdef COMMENT
    if (tn_sopt(WONT,TELOPT_ECHO) < 0)	/* I won't echo. */
      return(-1);
    if (tn_sopt(DO,TELOPT_ECHO) < 0)	/* Please, you echo. */
      return(-1);
#endif /* COMMENT */
    tn_init = 1;			/* Set flag to remember we did this. */
    debug(F100,"tn_ini ok","",0);
    return(1);
#endif /* TNCODE */
}
...
/* tn_doop */ {
    ...
    if (z != IAC) return(0);                    /* Character should be IAC */
...
#ifdef COMMENT
	    if (tn_sopt(DO,x) < 0)	/* Please do. */
	      return(-1);
	    else return(echo ? 2 : 0);
#else
...
#ifdef COMMENT
	    if (tn_sopt(DONT,x) < 0)	/* So don't. */
	      return(-1);
	    else return(echo ? 1 : 0);	/* I will. */
#else
	    if (!echo) {		/* If I'm not echoing already */
		if (tn_sopt(DONT,x) < 0) /* agree to echo. */
		  return(-1);
		else return(1);
	    } else return(0);
#endif /* COMMENT */
	...
	if (c == DO) {			/* Host wants me to echo */
	    if (tn_sopt(WONT,x) < 0)	/* I say I won't, */
	      return(-1);
	    if (tn_sopt(DO,x) < 0)	/* and ask the host to echo. */
	      return(-1);
            else return(echo ? 2 : 0);
	}
	...
	if (c == WONT) {		/* Host says it won't. */
		...
		if (tn_sopt(DONT,x) < 0) /* acknowledge, */
		  return(-1);
		else return(1);		/* and switch to local echo. */
	    ...
	    sgaflg = 0;			/* Remember. */
#ifdef COMMENT
	    if (tn_sopt(DO,x) < 0)	/* So do. */
	      return(-1);
	    else return(0);
#else
	    if (echo) {			/* If I'm echoing now, */
		if (tn_sopt(DO,x) < 0)	/* this is a change, so ACK. */
		  return(-1);
		else if (tn_sopt(DO,TELOPT_ECHO) < 0) /* Request remote echo */
		  return(-1);
		else return(0);		/* But don't switch yet */
	    }
#endif /* COMMENT */
	}
	return(0);
	...
	switch(c) {
	  ...
	  case DO:			/* You want me to? */
	    if (tn_sopt(WONT,x) < 0)	/* I won't. */
	      return(-1);
	    if (tn_sopt(DONT,x) < 0)	/* Don't you either. */
	      return(-1);
	    break;
	  case DONT:			/* (fall thru...) */
	    if (tn_sopt(WONT,x) < 0)	/* I won't. */
