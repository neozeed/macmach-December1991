 ...
 * HISTORY
 * $Log:	lance.h,v $
 * Revision 2.11  91/08/24  11:52:12  af
 * 	Fixed mis-typing of strides, mods to switch to cope with yet-another
 * 	DMA bogosity.
 * 	[91/08/02  01:54:00  af]
 * 
 * Revision 2.10  91/06/19  11:53:39  rvb
 * 	File moved here from mips/PMAX since it tries to be generic;
 * 	it is used on the PMAX and the Vax3100.
 * 	[91/06/04            rvb]
 * 
 * Revision 2.9  91/05/14  17:22:15  mrt
 * 	Correcting copyright
 * 
 * Revision 2.8  91/02/05  17:41:17  mrt
 * 	Changed to use new Mach copyright
 * 	[91/02/02  12:11:54  mrt]
 * 
 * Revision 2.7  90/12/05  23:31:29  af
 * 
 * 
 * Revision 2.6  90/12/05  20:46:57  af
 * 	Added decl of se_switch for kernel use.
 * 	[90/12/03  23:19:49  af]
 * 
 * Revision 2.5  90/09/09  23:20:30  rpd
 * 	Minor typos.
 * 	[90/09/09  19:00:09  af]
 * 
 * 	Documented the extra info that the mapped interface shares with 
 * 	the user-level driver.
 * 	[90/08/30  17:09:12  af]
 * 
 * Revision 2.4  90/08/27  22:05:53  dbg
 * 	Fixed copyright notice.
 * 	[90/08/17  23:39:45  af]
 * 
 * Revision 2.3  90/08/07  22:24:50  rpd
 * 	Re-Created from scratch, using AMD's data sheets.
 * 	[90/08/07  15:50:53  af]
 * 
 ...
 */
typedef volatile unsigned short *se_reg_t;
...
struct se_init_block {
	...
	unsigned char	recv_ring_pointer_hi,	/* Receive Ring ptr, high  */
			reserved0 : 5,
			recv_ring_len : 3;	/* Length: log2(nbuffers)  */
	unsigned short	xmit_ring_pointer_lo;	/* Transmit Ring ptr, low  */
	unsigned char	xmit_ring_pointer_hi,	/* Transmit Ring ptr, high */
			reserved1 : 5,
...
struct se_desc {
	...
	unsigned char	addr_hi,		/* (H)  Buffer pointer high */
...
extern struct se_switch {
	unsigned int	regspace;
	unsigned int	bufspace;
	unsigned int	ln_bufspace;
	unsigned int	romspace;
	short		romstride;
	short		ramstride;
	int		ramsize;
	int		(*desc_copyin)();
