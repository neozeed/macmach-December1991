	...
	color_map_t	*map;
{
	...
	for (i = 0; i < 16; i++, map++) {
		regs->addr_cmap_data = map->red;
		wbflush();
		regs->addr_cmap_data = map->green;
		wbflush();
		regs->addr_cmap_data = map->blue;
		wbflush();
	}
}
	...
	bt455_padded_regmap_t	*regs;
{
	...
	for (i = 0; i < 16; i++)
		bt455_load_colormap_entry(regs, 0, &m[0]);
	...
	color_map_t	*color;
{
	...
	/* Fg is overlay */
	color++;
	regs->addr_ovly = color->red;
	wbflush();
	regs->addr_ovly = color->green;
	wbflush();
	regs->addr_ovly = color->blue;
	wbflush();
}
