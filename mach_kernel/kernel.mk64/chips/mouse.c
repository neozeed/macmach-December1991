...
mouse_notify_mapped(unit, screen_unit, up)
	user_info_t	*up;
{
...
int kmin_mouse_problem = 0;

mouse_attach(unit, sl_unit)
{
	int		messg[4], s;
	mouse_softc_t	ms;
...
/* XXX split mouse and tablet, gets better Icache performance */
	...
	mouse_softc_t	ms;
{
	...
	if (flg & MS_TABLET) {
		...
		flg = DEV_TABLET;
#if 0
		x = ms->report.tb.xhi;	/* sign extends */
		x = (x << 8) | ms->report.xlo;
		y = ms->report.tb.yhi;	/* sign extends */
		y = (y << 8) | ms->report.ylo;
#else
		...
		y = ms->report.ms.ypos;
		if ((scale >= 0) && (y >= threshold))
			y += (y - threshold) * scale;
		if (ms->report.ms.sy)
			y = -y;

		/* we run the mouse in incremental mode */
		x += up->mouse_loc.x;
		y += up->mouse_loc.y;
	}
 ...
 */
mouse_button_event(ms, flg, bmask)
	mouse_softc_t	ms;
{
