...
#if	NORMA_IPC
	is_remote = (((ipc_port_t) dest)->ip_receiver == ipc_space_remote);
#endif	NORMA_IPC
			...
			for (i = 0; i < number; i++) {
				...
				if (is_remote) {
					...
					if (newname ==
						...
						panic("remote send rcv!\n");
					}
					ip_lock(ip);
					if (! ip->ip_norma_uid) {
						norma_ipc_export(object);
					}
...
#if	NORMA_IPC
	is_remote = (((ipc_port_t) remote)->ip_receiver == ipc_space_remote);
#endif	NORMA_IPC
	...
	while (saddr < eaddr) {
		...
		if (is_inline) {
			...
			if (is_remote) {
				kmsg->ikm_header.msgh_bits |=
				    MACH_MSGH_BITS_COMPLEX_DATA;
				vm_map_convert_to_page_list(data);
			}
#endif	NORMA_IPC
			saddr += sizeof(vm_offset_t);
		}
	...
	mach_msg_header_t *msgh;
{
	...
	db_printf("msgh->msgh_bits=0x%x", msgh->msgh_bits);
	...
	db_printf("msgh->msgh_id=%d, size=%d, kind=",
		  msgh->msgh_id,
		  msgh->msgh_size);
	if (msgh->msgh_kind == MACH_MSGH_KIND_NORMAL) {
		printf("normal\n");
	} else if (msgh->msgh_kind == MACH_MSGH_KIND_NOTIFICATION) {
		printf("notification\n");
	} else {
		printf("%d\n", msgh->msgh_kind);
	}
	...
	if (msgh->msgh_remote_port) {
		db_printf("remote=0x%x type=", msgh->msgh_remote_port);
	...
	if (msgh->msgh_local_port) {
		db_printf("local= 0x%x type=", msgh->msgh_local_port);
	...
	while (saddr < eaddr) {
		...
		printf(",size=%d,number=%d,addr=0x%x\n",
		       size,
		       number,
		       saddr);
