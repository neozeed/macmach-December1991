...
mach_msg_return_t
mach_msg_send(msg, option, send_size, timeout, notify)
...
mach_msg_return_t
mach_msg_receive(msg, option, rcv_size, rcv_name, timeout, notify)
	mach_msg_header_t *msg;
	mach_msg_option_t option;
...
mach_msg_receive_continue()
{
	...
	mach_msg_timeout_t timeout = self->ith_timeout;
	...
	mach_port_t notify;
{
	...
	if (option == (MACH_SEND_MSG|MACH_RCV_MSG)) {
		...
		if (copyinmsg((char *) msg, (char *) &kmsg->ikm_header,
			      send_size)) {
			ikm_free(kmsg);
			goto slow_get;
		}
...
#if	NORMA_IPC
			if (dest_port->ip_receiver == ipc_space_remote) {
				ip_unlock(dest_port);
		...
		assert(dest_port->ip_receiver != ipc_space_kernel);
#if	NORMA_IPC
		assert(dest_port->ip_receiver != ipc_space_remote);
#endif	NORMA_IPC
		...
		if ((kmsg->ikm_size != IKM_SAVED_KMSG_SIZE) ||
		    copyoutmsg((char *) &kmsg->ikm_header, (char *) msg,
			       reply_size) ||
		    (ikm_cache() != IKM_NULL))
			goto slow_put;
...
#if	NORMA_IPC
			    (dest_port->ip_receiver != ipc_space_remote) &&
#endif	NORMA_IPC
			    ((dest_port->ip_msgcount < dest_port->ip_qlimit) ||
...
msg_return_t
msg_send_trap(msg, option, send_size, timeout)
	msg_header_t *msg;
	msg_option_t option;
...
msg_return_t
msg_receive_trap(msg, option, rcv_size, rcv_name, timeout)
	msg_header_t *msg;
	msg_option_t option;
	msg_size_t rcv_size;
...
msg_receive_continue()
{
	...
	msg_timeout_t timeout = (msg_timeout_t) self->ith_timeout;
	ipc_object_t object = self->ith_object;
	ipc_mqueue_t mqueue = self->ith_mqueue;
	ipc_kmsg_t kmsg;
	mach_port_seqno_t seqno;
