...
kern_return_t
ipc_marequest_info(maxp, infop, countp)
	unsigned int *maxp;
	...
	mach_msg_type_number_t *countp;
{
	...
	kr = vm_allocate(ipc_kernel_map, (vm_offset_t *) &info,
			 (vm_size_t) (ipc_marequest_size * sizeof *info),
			 TRUE);
	if (kr != KERN_SUCCESS)
	...
	for (i = 0; i < ipc_marequest_size; i++) {
		...
		for (marequest = bucket->imarb_head, count = 0;
		     marequest != IMAR_NULL;
		     marequest = marequest->imar_next, count++)
			continue;
