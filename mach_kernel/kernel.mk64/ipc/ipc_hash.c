...
#define IH_GLOBAL_HASH(space, obj)			\
	(((((ipc_hash_index_t) (space)) >> 4) +		\
	  (((ipc_hash_index_t) (obj)) >> 6)) &		\
	...
	mach_msg_type_number_t *countp;
{
	...
	kr = vm_allocate(ipc_kernel_map, (vm_offset_t *) &info,
			 (vm_size_t) (ipc_hash_global_size * sizeof *info),
			 TRUE);
	if (kr != KERN_SUCCESS)
	...
	for (i = 0; i < ipc_hash_global_size; i++) {
		...
		for (entry = bucket->ihgb_head, count = 0;
		     entry != ITE_NULL;
		     entry = entry->ite_next, count++)
			continue;
