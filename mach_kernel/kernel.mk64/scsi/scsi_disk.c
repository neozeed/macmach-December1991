...
scdisk_mode_select( sc, tgt, lbn, ior, mdata, mlen, save)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	register int 		lbn;
	io_req_t		ior;
	char			*mdata;
{
	...
	scsi_go_and_wait(sc, tgt, sizeof(*cmd) + mlen, 0, ior);
...
scsi_reassign_blocks( sc, tgt, defect_list, n_defects, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	...
	io_req_t		ior;
{
	...
	(*sc->go)(sc, tgt, sizeof(*cmd) + sizeof(*parm) + (n_defects - 4), 0, FALSE);
}

scsi_medium_removal( sc, tgt, allow, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	boolean_t		allow;
	io_req_t		ior;
...
scsi_format_unit( sc, tgt, mode, vuqe, intlv, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	...
	io_req_t		ior;
{
	...
	scsi_go_and_wait(sc, tgt, sizeof(*cmd) + ior->io_count, 0, ior);
	return tgt->done;
}
...
scsi_long_read( sc, tgt, secno, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	register unsigned int	secno; 
	io_req_t		ior;
...
scsi_long_write( sc, tgt, secno, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	register unsigned int	secno;
	io_req_t		ior;
...
scdisk_verify( sc, tgt, secno, nsectrs, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	io_req_t		ior;
{
	...
	scsi_go_and_wait(sc, tgt, sizeof(*cmd) + len, 0, ior);
	return tgt->done;
}


scsi_read_defect( sc, tgt, mode, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
...
scsi_reserve( sc, tgt, len, id, mode, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
...
scsi_release( sc, tgt, id, mode, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
...
scsi_long_seek( sc, tgt, secno, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	io_req_t		ior;
{
...
scsi_write_verify( sc, tgt, secno, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	io_req_t		ior;
{
...
scsi_search_data( sc, tgt, secno, how, flags, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	io_req_t		ior;
{
...
scsi_set_limits( sc, tgt, secno, nblocks, inhibit, ior)
	register scsi_softc_t	*sc;
	register target_info_t	*tgt;
	io_req_t		ior;
{
