...
#define SPC_WAIT_PHASE_VANISH(p) while(1) { int psns = regs->spc_psns; \
		            if ((psns & SPC_BUS_REQ) && (psns & SCSI_PHASE_MASK)!=p) break; }
...
int	spc_probe(), scsi_slave(), scsi_attach(), spc_go(), spc_intr();
...
spc_script_state(unit)
{
	...
	db_printsym(spt->action,1);
	db_printf(", ");
	db_printsym(spc_softc[unit]->error_handler, 1);
	return 0;
	...
	struct bus_ctlr	*ui;
{
	...
	for (target_id = 0; target_id < 8; target_id++, cmd_ptr += PER_TGT_BUFF_DATA) {
		...
		register unsigned csr, dsr, ints;
		...
		csr = spc_data_in(regs, SCSI_PHASE_MSG_IN, 1, &i);
		LOG(0x25,"din_count");
		LOG(0x80+csr,0);
		...
		spc->ntargets++;
		{
			...
			tgt = scsi_slave_alloc(unit, target_id, spc);
...
boolean_t
spc_probe_target(sc, tgt, ior)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	io_req_t		ior;
{
	spc_softc_t     spc = spc_softc[sc->masterno];
	boolean_t	newlywed;
	...
	if (newlywed) {
		...
		tgt->cmd_ptr = &spc_buffer[PER_TGT_BUFF_DATA*tgt->target_id +
					   (sc->masterno*8*PER_TGT_BUFF_DATA) ];
		/* "physical" address for dma engine */
		tgt->dma_ptr = 0;
#ifdef	MACH_KERNEL
#else	/*MACH_KERNEL*/
...
nodev:
	...
	mask = regs->spc_ints; 
	regs->spc_ints = mask; /* noop */
	/* regs->spc_scmd = 0; target only */
	ret = SCSI_RET_DEVICE_DOWN;
	return ret;
}

spc_data_out(regs, phase, count, data)
	register spc_regmap_t	*regs;
	unsigned char		*data;
{
        int ret;
...
spc_data_in(regs, phase, count, data)
	register spc_regmap_t	*regs;
	unsigned char		*data;
{
        int ret;
 ...
 */
spc_go(sc, tgt, cmd_count, in_count, cmd_only)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	boolean_t		cmd_only;
{
...
you`ll have to special case this 
	...
	disconn  = BGET(scsi_might_disconnect,spc->sc->masterno,tgt->target_id);
	disconn  = disconn && (spc->ntargets > 1);
	disconn |= BGET(scsi_should_disconnect,spc->sc->masterno,tgt->target_id);
...
spc_intr(unit)
{
	...
	register unsigned	ints, psns, ssts, serr; 
	register spc_regmap_t	*regs;
	boolean_t		try_match;
#if	notyet
...
#endif	/*MACH_KERNEL*/
		LOG(0xA,"ops->restart");
		(*tgt->dev_ops->restart)( spc->sc, tgt, TRUE);
		if (csr)
		  spc->state &= ~SPC_STATE_BUSY;
	}
...
msgin:
	...
	{
	  register int ints = regs->spc_ints;
	  regs->spc_ints = ints;
	}
...
boolean_t
 spc_err_generic(spc, ints, psns, ssts)
	register spc_softc_t	spc;
{
	register int		cond = spc->script->condition;
	register spc_regmap_t  *regs = spc->regs; 
