...
boolean_t
sii_probe_target(sc, tgt, ior)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	io_req_t		ior;
{
	sii_softc_t     sii = sii_softc[sc->masterno];
	boolean_t	newlywed;
	...
	if (newlywed) {
		...
		tgt->dma_ptr = dma_ptr;
		tgt->cmd_ptr = decent_buffer[sc->masterno*8 + tgt->target_id];
#ifdef	MACH_KERNEL
#else	/*MACH_KERNEL*/
		fdma_init(&tgt->fdma, scsi_per_target_virtual);
#endif	/*MACH_KERNEL*/
 ...
 */
sii_go(sc, tgt, cmd_count, in_count, cmd_only)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	boolean_t		cmd_only;
{
	...
	disconn  = BGET(scsi_might_disconnect,sii->sc->masterno,tgt->target_id);
	disconn  = disconn && (sii->ntargets > 1);
	disconn |= BGET(scsi_should_disconnect,sii->sc->masterno,tgt->target_id);
	...
	register sii_softc_t	sii;
{
	...
	if (BGET(scsi_no_synchronous_xfer,sii->sc->masterno,tgt->target_id))
		off = 0;
	else
		off = sii_max_offset;
