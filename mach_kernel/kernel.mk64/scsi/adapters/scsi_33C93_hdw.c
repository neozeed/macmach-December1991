...
boolean_t
sbic_probe_target(sc, tgt, ior)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	io_req_t		ior;
{
	sbic_softc_t     sbic = sbic_softc[sc->masterno];
	boolean_t	newlywed;
 ...
 */
sbic_go(sc, tgt, cmd_count, in_count, cmd_only)
	scsi_softc_t		*sc;
	target_info_t		*tgt;
	boolean_t		cmd_only;
{
	...
	disconn  = BGET(scsi_might_disconnect,sbic->sc->masterno,tgt->target_id);
	disconn  = disconn && (sbic->ntargets > 1);
	disconn |= BGET(scsi_should_disconnect,sbic->sc->masterno,tgt->target_id);
	...
	register sbic_softc_t	sbic;
{
	...
	if (ior = tgt->ior) {
		...
		LOG(0xA,"ops->restart");
		(*tgt->dev_ops->restart)( sbic->sc, tgt, TRUE);
	}
