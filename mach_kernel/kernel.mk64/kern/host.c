...
kern_return_t host_processors(host, processor_list, countp)
	host_t			host;
	...
	long			*countp;
{
	...
	*processor_list = (mach_port_t *) addr;
	...
	tp = (processor_t *) addr;
	for (i = 0; i < count; i++)
		((mach_port_t *) tp)[i] = convert_processor_to_port(tp[i]);
...
unsigned int			*count;
{
	...
	*count = actual;
	...
	for (i = 0; i < actual; i++)
		((mach_port_t *) psets)[i] =
			convert_pset_name_to_port(psets[i]);
...
unsigned int			*count;
{
	...
	/* do the conversion that Mig should handle */
	*((mach_port_t *) addr) = convert_pset_name_to_port(&default_pset);
