	...
	vm_size_t memory_object_page_size;
{
	...
	KOBJ->prot = (vm_prot_t *) kalloc(MOBJ->num_pages * sizeof(vm_prot_t));
#if 9
...
Mcheck
if(xmm_debug_allowed)printf("k_svm_lock_request!\n");

	if (length != vm_page_size) {
if(xmm_debug_allowed)printf("kern . failure %d %x %x\n", __LINE__, length, vm_page_size);
		if (length < vm_page_size) {
			length = vm_page_size;
			if(xmm_debug_allowed)printf("... Saved!\n");
		} else {
			return KERN_FAILURE;
		}
	}
...
#if 0
if(xmm_debug_allowed)printf("kern . failure\n");
		return KERN_FAILURE;
#else
	 ...
	 */
if(xmm_debug_allowed)printf("** lock_request: submitting request\n");
	r = (request_t) zalloc(xmm_svm_request_zone);
	 ...
	 */
	if (! r0) {
if(xmm_debug_allowed)printf("- no reqs\n");
	 ...
	 */
	if (! r0->is_kernel) {
if(xmm_debug_allowed)printf("- only pager reqs\n");
...
if(xmm_debug_allowed)printf("add_request: 0x%x 0x%x\n", r, r0);
