...
#define	io_data		io_un.data
	...
	decl_simple_lock_data(,io_lock)	/* Lock for this structure */
};
...
#define ior_lock(ior)	simple_lock(&(ior)->io_lock)
#define ior_unlock(ior)	simple_unlock(&(ior)->io_lock)
...
#define	io_req_alloc(ior,size)					\
	...
	simple_lock_init(&(ior)->io_lock);			\
	MACRO_END

#define	io_req_free(ior)					\
	(kfree((char *)(ior), sizeof(struct io_req)))
