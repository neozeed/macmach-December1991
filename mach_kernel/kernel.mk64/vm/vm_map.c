	...
	boolean_t	interruptible;
{
	...
	assert(page_aligned(copy->offset));
	assert(page_aligned(copy->size));
...
kern_return_t	vm_map_copyin_page_list_cont(cont_args, copy_result, abort)
vm_map_copyin_args_t	cont_args;
vm_map_copy_t		*copy_result;	/* OUT */
boolean_t		abort;
{
	kern_return_t	result;
	boolean_t	src_destroy, src_destroy_only;
	...
	if (abort || src_destroy_only) {
		result = vm_map_remove(cont_args->map, cont_args->src_addr,
 ...
 */
kern_return_t
vm_map_convert_to_page_list(copy)
	vm_map_copy_t	copy;
{
	...
	vm_object_t object;
	kern_return_t result;
	vm_prot_t req_fault_type = VM_PROT_READ;
	vm_prot_t result_prot;
	vm_page_t top_page;
	vm_page_t copy_page_list[VM_MAP_COPY_PAGE_LIST_MAX];
	int i, num_pages = 0;
	...
	if (copy->type == VM_MAP_COPY_OBJECT) {
		return vm_map_convert_to_page_list_from_object(copy);
	}
	...
	/*
	 * Find the zone that the entries were allocated from,
	 * so that we can free them correctly.
	 */
	if (copy->cpy_hdr.entries_pageable) {
		entry_zone = vm_map_entry_zone;
	} else {
		entry_zone = vm_map_kentry_zone;
	}
	...
	for (entry = vm_map_copy_first_entry(copy);
		...
		for (va = entry->vme_start;
			...
			vm_page_t m;
			int i = (va - entry->vme_start) / PAGE_SIZE;

			if (num_pages == VM_MAP_COPY_PAGE_LIST_MAX) {
...
retry:
			...
			if (result != VM_FAULT_SUCCESS) {
				/* XXX (also io_wire) what about data_error? */
				vm_object_lock(object);
				vm_object_paging_begin(object);
				goto retry;
			}

			/* If the object is a permanent object... */
	...
	/*
	 * Free entries.
	 * We should probably have done this in loop above.
	 */
	for (entry = vm_map_copy_first_entry(copy);
	     entry != vm_map_copy_to_entry(copy);
	     entry = next_entry) {
		next_entry = entry->vme_next;
		zfree(entry_zone, (vm_offset_t) entry);
	}
	 ...
	 */
	copy->type = VM_MAP_COPY_PAGE_LIST;
	copy->cpy_npages = num_pages;
	for (i = 0; i < num_pages; i++) {
		copy->cpy_page_list[i] = copy_page_list[i];
	}
	copy->cpy_cont = ((kern_return_t (*)()) 0);
	copy->cpy_cont_args = (char *) VM_MAP_COPYIN_ARGS_NULL;
	return KERN_SUCCESS;
...
kern_return_t
vm_map_convert_to_page_list_from_object(copy)
	vm_map_copy_t copy;
{
	vm_object_t object;
	int i;
	...
	copy->type = VM_MAP_COPY_PAGE_LIST;
	copy->cpy_npages = object->size / PAGE_SIZE;
	...
	for (i = 0; i < copy->cpy_npages; i++) {
		...
		m = vm_page_lookup(object, i * PAGE_SIZE);
		if (m) {
			/* XXX mark busy */
			copy->cpy_page_list[i] = m;
			vm_object_paging_end(object);
			vm_object_unlock(object);
			continue;
		}
retry:
		result_prot = VM_PROT_READ;
		result = vm_fault_page(object, i * PAGE_SIZE,
				       VM_PROT_READ, FALSE, FALSE,
				       &result_prot, &m, &top_page,
				       FALSE, (void (*)()) 0);
		if (result == VM_FAULT_MEMORY_SHORTAGE) {
	...
	vm_map_copy_t	copy;
{
	...
	for (i = 0; i < copy->cpy_npages; i++) {
		register vm_page_t m = copy->cpy_page_list[i];
