 ...
 * HISTORY
 * $Log:	srr.c,v $
 * Revision 2.1  90/10/27  20:46:22  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.19.1.1.1.2  90/08/02  20:24:08  dpj
 * 	Reorganized for new netipc interface and packet format.
 * 	[90/06/03  17:42:54  dpj]
 * 
 * Revision 1.19.1.1.1.1  90/07/30  14:06:43  dpj
 * 	Added filter priority in netipc_bind().
 * 
 * Revision 1.19  89/05/02  11:17:35  dpj
 * 	Fixed all files to conform to standard copyright/log format
 * 
 * Revision 1.18  89/04/24  20:42:26  dpj
 * 	Changes from NeXT as of 88/09/30
 * 	[89/04/19  17:57:45  dpj]
 * 
...
#ifndef	lint
char srr_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/srr.c,v 2.1 90/10/27 20:46:22 dpj Exp $";
#endif not lint
...
netipc_channel_ptr_t		srr_channel;
static cthread_t		srr_listen_thread;


PRIVATE void srr_main();
...
BEGIN("srr_init")
    ...
    srr_channel = netipc_bind(SRR_UDP_PORT,CRYPT_HEADER_SIZE,NETIPC_PRI_LOW);
    if (srr_channel == NULL) {
	ERROR((msg, "srr_init.netipc_bind fails."));
	RETURN(FALSE);
    }

    /*
     * Now fork a thread to execute the receive loop of
     * the srr transport protocol.
     */
...
BEGIN("srr_main")
	 ...
	 */
	kr = netipc_receive(srr_channel,(netipc_ptr_t)in_packet_ptr);
	if (kr != RCV_SUCCESS) {
	    ERROR((msg, "srr_main.netipc_receive fails, kr = %d.", kr));
	}
	else {
	    crypt_level = ntohl(in_packet_ptr->crypt_header.ch_crypt_level);
	    if (crypt_level < 0) {
		/*
		...
		if (crypt_remote_failure) {
		    /*
		     * This packet was sent by us but could not be decrypted
		     * by the remote network server.
		     */
		    LOG0(TRUE, 5, 1055);
		    ...
		    data_size =
			in_packet_ptr->srr_pkt_header.control.data_size - 
					SRR_HEADER_SIZE - CRYPT_HEADER_SIZE;
...
BEGIN("srr_retry")
	...
	if (host_info->shi_request_status == SRR_AWAITING_RESPONSE) {
	    ...
	    kr = netipc_send(srr_channel,(netipc_ptr_t)request_packet_ptr,
