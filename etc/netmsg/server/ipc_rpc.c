 ...
 * HISTORY
 * $Log:	ipc_rpc.c,v $
 * Revision 2.1  90/10/27  20:44:19  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.12.1.1.1.3  90/08/15  14:57:53  dpj
 * 	Updated for MACH3_SA.
 * 
 * Revision 1.12.1.1.1.2  90/08/02  20:22:17  dpj
 * 	Use explicit port set instead of port_enable/port_disable.
 * 	[90/06/03  17:33:03  dpj]
 * 
 * Revision 1.12.1.1.1.1  90/07/30  13:53:58  dpj
 * 	Always ignore the crypt_level if USE_CRYPT is false.
 * 	Replaced one call to mach_error() with ERROR() for standalone operation.
 * 
 * Revision 1.12  89/05/02  11:10:08  dpj
 * 	Fixed all files to conform to standard copyright/log format
 * 
 * Revision 1.11  89/04/24  20:39:51  dpj
 * 	Changes from NeXT as of 88/09/30
 * 	[89/04/19  17:53:32  dpj]
 * 
 ...
 */
#ifndef	lint
char ipc_rpc_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/ipc_rpc.c,v 2.1 90/10/27 20:44:19 dpj Exp $";
#endif not lint
/*
 *
 */
...
port_t nmserver_port_set = PORT_NULL;
...
/*		DEBUG2(debug.ipc_out,0,2583,log_cur_ptr,log_end_ptr); */
		...
		rcvbuff->msg_local_port = nmserver_port_set;
...
#else	USE_CRYPT
	crypt_level = CRYPT_DONT_ENCRYPT;	/* XXX */
#endif	USE_CRYPT
	...
	if (retry) {
		...
		if (server_port_ptr->portrec_waiting_count > 0) {
			...
			if ((server_port_ptr->portrec_transit_count == 0) &&
				...
				kern_ret = port_set_add(task_self(),
					nmserver_port_set,
				...
				if (kern_ret == KERN_SUCCESS) {
					...
					ERROR((msg,"port_set_add() returned %d",
									kern_ret));
				}
			}
		}
 ...
 *	TRUE or FALSE.
 *
 * Allocate nmserver_port_set.
 */
PUBLIC boolean_t port_set_init()
BEGIN("port_set_init")
	kern_return_t	kern_ret;

	kern_ret = port_set_allocate(task_self(), &nmserver_port_set);
	if (kern_ret != KERN_SUCCESS) {
		ERROR((msg,"port_set_allocate: kr=%d", kern_ret));
		RETURN(FALSE);
	} else {
		RETURN(TRUE);
	}
...
BEGIN("ipc_rpc_init")
	...
	ipc_sequence_number = 20 * (tp.seconds - (17 * 365 * 24 * 60 * 60));
#else	MACH3_SA || MACH3_US
	 ...
	 */
	new_thread = cthread_fork(ipc_out_main, 0);
	cthread_set_name(new_thread, "ipc_out_main");
	cthread_detach(new_thread);

	new_thread = cthread_fork(ipc_re_send, 0);
