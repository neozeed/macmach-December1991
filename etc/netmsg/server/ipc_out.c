 ...
 * HISTORY
 * $Log:	ipc_out.c,v $
 * Revision 2.1  90/10/27  20:44:16  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.33.1.1.1.2  90/08/02  20:22:06  dpj
 * 	Use explicit port set instead of port_enable/port_disable.
 * 	[90/06/03  17:32:36  dpj]
 * 
 * Revision 1.33.1.1.1.1  90/07/30  13:53:04  dpj
 * 	Always ignore the crypt_level if USE_CRYPT is false.
 * 
 * Revision 1.33  89/05/02  11:09:46  dpj
 * 	Fixed all files to conform to standard copyright/log format
 * 
 * Revision 1.32  89/04/24  20:39:30  dpj
 * 	Changes from NeXT as of 88/09/30
 * 	[89/04/19  17:53:19  dpj]
 * 
 ...
 */
#ifndef	lint
char ipc_out_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/ipc_out.c,v 2.1 90/10/27 20:44:16 dpj Exp $";
#endif not lint
/*
...
extern port_t nmserver_port_set;
	...
	case IPC_SUCCESS: {
		...
		if ((dest_port_ptr->portrec_current_ipcrec = dest_port_ptr->portrec_backlog_ipcrec) != 0) {
			...
			if (!(dest_port_ptr->portrec_info & (PORT_INFO_SUSPENDED | PORT_INFO_BLOCKED))) {
				kern_ret = port_set_add(task_self(), nmserver_port_set, dest_port_ptr->portrec_local_port);
				if (kern_ret == KERN_SUCCESS) {
					dest_port_ptr->portrec_info &= ~PORT_INFO_DISABLED;
				} else {
					ERROR((msg, "ipc_out_cleanup: port_set_add returned %d.", kern_ret));
				}
			}
		}
...
#else	USE_CRYPT
ior_ptr->out.crypt_level = CRYPT_DONT_ENCRYPT;	/* XXX */
#endif	USE_CRYPT
...
if (kern_ret == KERN_SUCCESS) {
	...
	ERROR((msg, "ipc_out_backlog: port_set_remove returned %d", kern_ret));
}
...
if (dest_port_ptr->portrec_current_ipcrec == 0) {
	...
	(void)ipc_retry(dest_port_ptr);
	if (!(dest_port_ptr->portrec_info & (PORT_INFO_SUSPENDED | PORT_INFO_BLOCKED))) {
		if (kern_ret = port_set_add(task_self(), nmserver_port_set, dest_port_ptr->portrec_local_port)
			== KERN_SUCCESS)
		{
			dest_port_ptr->portrec_info &= ~PORT_INFO_DISABLED;
		}
		else {
			ERROR((msg, "ipc_out_backlog: port_set_add returned %d", kern_ret));
		}
	}
}
