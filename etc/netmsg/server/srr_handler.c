 ...
 * HISTORY
 * $Log:	srr_handler.c,v $
 * Revision 2.1  90/10/27  20:46:32  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.17.2.1  90/08/02  20:24:17  dpj
 * 	Reorganized for new netipc interface and packet format.
 * 	[90/06/03  17:43:35  dpj]
 * 
 ...
 */
#ifndef	lint
char srr_handler_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/srr_handler.c,v 2.1 90/10/27 20:46:32 dpj Exp $";
#endif not lint
/*
...
BEGIN("srr_handle_crypt_failure")
     ...
     */
    host_id = in_packet_ptr->srr_pkt_header.control.from;
...
BEGIN("srr_handle_request")
	...
	source_host = in_packet_ptr->srr_pkt_header.control.from;
	...
	if (kr == DISP_FAILURE) {
	    ...
	    if (kr == CRYPT_SUCCESS) {
		if ((kr = netipc_send(srr_channel,
			(netipc_header_ptr_t)in_packet_ptr,
			CRYPT_NEED_CKSUM(in_packet_ptr))) != KERN_SUCCESS) {
	...
	if (broadcast) {
	    if ((kr = netipc_send(srr_channel,
		(netipc_header_ptr_t)host_info->shi_bcast_response_packet,
		CRYPT_NEED_CKSUM(host_info->shi_bcast_response_packet)))
		!= KERN_SUCCESS)
	    {
		ERROR((msg, "srr_handle_request.netipc_send fails, kr = %d.", kr));
	    }
	    ...
	    if ((kr = netipc_send(srr_channel,
		(netipc_header_ptr_t)host_info->shi_response_packet,
		CRYPT_NEED_CKSUM(host_info->shi_response_packet)))
		!= KERN_SUCCESS) {
		ERROR((msg, "srr_handle_request.netipc_send fails, kr = %d.", kr));
	    }
	    else INCSTAT(srr_replies_sent);
...
BEGIN("srr_process_queued_request")
	 ...
	 */
	crypt_level = ntohl(request_packet_ptr->crypt_header.ch_crypt_level);
	if (crypt_level != CRYPT_DONT_ENCRYPT) {
	...
	if (kr == CRYPT_SUCCESS) {
	     ...
	     */
	    if ((kr = netipc_send(srr_channel,
		(netipc_header_ptr_t)request_packet_ptr,
		CRYPT_NEED_CKSUM(request_packet_ptr)))
		!= KERN_SUCCESS) {
...
BEGIN("srr_send_crypt_failure")
    ...
    in_packet_ptr->srr_pkt_header.control.to =
				in_packet_ptr->srr_pkt_header.control.from;
    in_packet_ptr->srr_pkt_header.control.from = 0;
    in_packet_ptr->crypt_header.ch_crypt_level = htonl((unsigned long)-crypt_level);

    if ((kr = netipc_send(srr_channel,(netipc_header_ptr_t)in_packet_ptr,FALSE)) != KERN_SUCCESS) {
