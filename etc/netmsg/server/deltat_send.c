 ...
 * HISTORY
 * $Log:	deltat_send.c,v $
 * Revision 2.1  90/10/27  20:43:59  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.20.2.1  90/08/02  20:21:38  dpj
 * 	Reorganized for new netipc interface and packet format.
 * 	[90/06/03  17:31:32  dpj]
 * 
 ...
 */
#ifndef	lint
char deltat_send_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/deltat_send.c,v 2.1 90/10/27 20:43:59 dpj Exp $";
#endif not lint
/*
...
BEGIN("deltat_send_abort")
	 ...
	 */
	if ((kr = netipc_send(deltat_channel,(netipc_ptr_t)pkt_ptr,CRYPT_NEED_CKSUM(pkt_ptr))) != KERN_SUCCESS) {
...
BEGIN("deltat_send_ack")
	 ...
	 * Actually try to send this packet.
	 */
	if ((kr = netipc_send(deltat_channel,(netipc_ptr_t)ack_pkt_ptr,CRYPT_NEED_CKSUM(ack_pkt_ptr))) != KERN_SUCCESS) {
...
BEGIN("deltat_send_crypt_failure")
    ...
    in_packet_ptr->deltat_pkt_header.control.to = 
				in_packet_ptr->deltat_pkt_header.control.from;
    in_packet_ptr->deltat_pkt_header.control.from = 0;
    in_packet_ptr->crypt_header.ch_crypt_level = htonl((unsigned long)-crypt_level);

    if ((kr = netipc_send(deltat_channel,(netipc_ptr_t)in_packet_ptr,FALSE)) != KERN_SUCCESS) {
