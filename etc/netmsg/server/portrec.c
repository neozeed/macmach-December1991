 ...
 * HISTORY
 * $Log:	portrec.c,v $
 * Revision 2.1  90/10/27  20:45:53  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.28.2.2  90/08/15  15:00:02  dpj
 * 	Updated for MACH3_SA.
 * 
 * Revision 1.28.2.1  90/08/02  20:23:57  dpj
 * 	Use explicit port set instead of port_enable/port_disable.
 * 	[90/06/03  17:41:34  dpj]
 * 
...
#ifndef	lint
char portrec_rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/server/portrec.c,v 2.1 90/10/27 20:45:53 dpj Exp $";
#endif not lint
...
#if	MACH3_SA || MACH3_US
	(void)mach_get_time(&now);
	unique_id = now.seconds;
#else	MACH3_SA || MACH3_US
...
BEGIN("pr_ntran")
	...
	if (p == (cthread_queue_item_t)NULL) {
		...
		(void)port_set_add(task_self(), nmserver_port_set, lport);
