 ...
 * HISTORY
 * $Log:	nmcontrol.c,v $
 * Revision 2.1  90/10/27  20:47:14  dpj
 * Moving under MACH3 source control
 * 
 * Revision 1.8.1.1  90/07/30  14:08:09  dpj
 * 	Accept an IP address of the form a.b.c.d as a machine name.
 * 	Do not call gethostbyname(), to simplify standalone operation.
 * 
 ...
 */
#ifndef	lint
static char     rcsid[] = "$Header: /afs/cs.cmu.edu/project/mach-2/rcs/pkg/netmsg/utils/nmcontrol.c,v 2.1 90/10/27 20:47:14 dpj Exp $";
#endif not lint
/*
...
char *host_name;
{
	...
	if (sscanf(host_name,"0x%x",&ip_addr.ia_netaddr) == 1) {
		return(ip_addr.ia_netaddr);
	}
	...
	hp = (struct hostent *) gethostbyname(host_name);
	if (hp == 0) return 0;
...
BEGIN("dumpportstat")
	...
	int     i;
	char *lptr;
	netaddr_t	machine_id = host_address(machine_name);
