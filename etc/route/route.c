...
flushroutes()
{
  fprintf(stderr, "route: flush disabled for mac2\n");
}
...
again:
	...
	if (doinghost) {
		lseek(kmem, nl[N_RTNET].n_value + kbase, 0);
		read(kmem, routehash, rthashsize*sizeof (struct mbuf *));
		doinghost = 0;
		goto again;
	}
